...
public class AutonomousPathCommand extends CommandBase {
    ...
    public void initializeRobotForPath() {
        KochanekBartelsSpline.PathPoint point = pathFollower.getPointAt(0.0);
        if (point != null) {
    ...
    public void execute() {
        ...
        KochanekBartelsSpline.PathPoint point = pathFollower.getPointAt(pathTime);
        if (point == null) {
            isFinished = true;
            ...
            if ((null != point.action) && (null != point.action.command)) {
                ...
                String commandClass = "frc.robot.commands." + point.action.command;
                ...
                try {
                    ...
                    if (KochanekBartelsSpline.RobotActionType.SCHEDULE_COMMAND == point.action.actionType) {
                        // this one is really simple - we just schedule the command, and it happens in
                        // parallel with path following.
                        CommandScheduler.getInstance().schedule(command);
                    } else if (KochanekBartelsSpline.RobotActionType.STOP_AND_RUN_COMMAND == point.action.actionType) {
            ...
            double errorRotation = (point.fieldHeading.getRadians() - NavX.getInstance().getHeading().getRadians()) *
